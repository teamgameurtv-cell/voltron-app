...
class QuoteScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final order = ref.watch(repairsProvider).firstWhere((o) => o.dbId == orderId);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(quote.date, style: const TextStyle(color: VoltronColors.greyText, fontSize: 12)),
                ...
                child: Column(
                  children: [
                    const Align(
                      ...
                      child: Text('DÉTAIL DE L\'INTERVENTION',
                          style: TextStyle(
                            fontSize: 11,
                            letterSpacing: 1,
                    ...
                    ...quote.lines.map((line) => Padding(
                          padding: const EdgeInsets.symmetric(vertical: 6),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(line.label, style: const TextStyle(fontSize: 13)),
                              Text('${line.price.toStringAsFixed(2).replaceAll('.', ',')} €',
              ...
              if (quote.status != QuoteStatus.pendingApproval) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (quote.status == QuoteStatus.accepted ? VoltronColors.success : const Color(0xFFFF5C5C))
                        .withValues(alpha: 0.15),
                    borderRadius: BorderRadius.circular(VoltronRadii.sm),
                  ),
                  child: Text(
                    quote.status == QuoteStatus.accepted ? 'Devis accepté — réparation en cours' : 'Devis refusé',
                    style: TextStyle(
                      color: quote.status == QuoteStatus.accepted ? VoltronColors.success : const Color(0xFFFF5C5C),
                      fontWeight: FontWeight.w700,
                      fontSize: 12,
                ...
                onPressed: quote.status == QuoteStatus.pendingApproval
                    ? () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Devis accepté, ta réparation avance !')),
                        );
                      }
                    : null,
              ...
              OutlinedButton(
                onPressed: quote.status == QuoteStatus.pendingApproval
                    ? () {
                        ref.read(repairsProvider.notifier).refuseQuote(order.dbId);
                        context.pop();
                        ScaffoldMessenger.of(context).showSnackBar(
