...
class RepairOrderCard extends ConsumerWidget {
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Expanded(
                child: Text('Dossier #${order.id}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
              ),
              if (order.quote != null)
                TextButton.icon(
                  onPressed: () => _showQuoteDialog(context, ref, order, existing: order.quote),
                  icon: const Icon(Icons.edit_outlined, size: 14),
                  label: const Text('MODIFIER LE DEVIS', style: TextStyle(fontSize: 11)),
                ),
              if (!isComplete && order.isBlockedOnQuote)
                ...
                ElevatedButton(
                  onPressed: () => _showQuoteDialog(context, ref, order),
          ...
          Wrap(
            ...
            children: order.steps.map((step) {
              ...
              return GestureDetector(
                onTap: () => _showStepNoteDialog(context, ref, step),
                child: Tooltip(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(step.label, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w600)),
                        if (hasNote) ...[
  ...
  void _showStepNoteDialog(BuildContext context, WidgetRef ref, RepairStep step) {
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: TextField(
            ...
            maxLines: 4,
            decoration: const InputDecoration(hintText: 'Note interne ou visible par le client'),
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ref.read(repairsProvider.notifier).updateStepNote(
                    step.id,
  ...
  void _showQuoteDialog(BuildContext context, WidgetRef ref, RepairOrder order, {Quote? existing}) {
    final delayController = TextEditingController(text: existing?.estimatedDelay ?? '');
    final lines = <(TextEditingController, TextEditingController)>[
      if (existing != null && existing.lines.isNotEmpty)
        for (final l in existing.lines)
          (TextEditingController(text: l.label), TextEditingController(text: l.price.toStringAsFixed(2)))
      else
              ...
              child: Column(
                ...
                children: [
                  TextField(
                    ...
                    decoration: const InputDecoration(hintText: 'Délai estimé (ex : 3-5 jours)'),
                  ),
                  const SizedBox(height: 14),
                  const Text('LIGNES DU DEVIS',
                      style: TextStyle(fontSize: 11, letterSpacing: 1, color: VoltronColors.greyText, fontWeight: FontWeight.w700)),
                    ...
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            flex: 3,
                            child: TextField(controller: labelCtrl, decoration: const InputDecoration(hintText: 'Intitulé')),
                          ),
                          ...
                          IconButton(
                            onPressed: () => setDialogState(() => lines.removeAt(entry.key)),
                            icon: const Icon(Icons.close, size: 16, color: VoltronColors.greyText),
                          ),
                        ],
                      ),
                    );
                  ...
                  TextButton.icon(
                    onPressed: () => setDialogState(() => lines.add((TextEditingController(), TextEditingController()))),
                    icon: const Icon(Icons.add, size: 16),
                    label: const Text('Ajouter une ligne'),
                  ),
                  const SizedBox(height: 10),
                  const Text('PIÈCE JOINTE (facultatif)',
                      style: TextStyle(fontSize: 11, letterSpacing: 1, color: VoltronColors.greyText, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      OutlinedButton.icon(
                        ...
                        icon: isUploading
                            ? const SizedBox(
                                width: 14,
                                height: 14,
                                child: CircularProgressIndicator(strokeWidth: 2, color: VoltronColors.electricYellow),
                              )
                            : const Icon(Icons.attach_file, size: 16),
                        label: Text(isUploading ? 'Envoi...' : (fileUrl != null ? 'Remplacer le fichier' : 'Joindre un fichier')),
                      ),
                      if (fileUrl != null) ...[
                        const SizedBox(width: 10),
                        const Icon(Icons.check_circle, color: VoltronColors.success, size: 16),
                      ],
                    ],
                  ),
                ],
              ...
              onPressed: isUploading
                  ...
                  : () {
                      final quoteLines = lines
                          ...
                          .map((l) => QuoteLine(l.$1.text.trim(), double.tryParse(l.$2.text.replaceAll(',', '.')) ?? 0))
                          .toList();
                      if (existing == null) {
                        ref.read(repairsProvider.notifier).createQuote(
                              order.dbId,
                              lines: quoteLines,
                        ...
                        ref.read(repairsProvider.notifier).updateQuote(
                              existing.dbId,
                              lines: quoteLines,
                              estimatedDelay: delayController.text.trim(),
                              fileUrl: fileUrl,
