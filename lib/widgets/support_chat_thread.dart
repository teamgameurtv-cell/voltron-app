...
/// Fil de discussion d'un ticket support, utilisé côté client et côté admin
/// (seul [myRole] change selon qui l'affiche).
class SupportChatThread extends ConsumerStatefulWidget {
  final String ticketId;
  final SenderRole myRole;

  const SupportChatThread({super.key, required this.ticketId, required this.myRole});

  @override
  ConsumerState<SupportChatThread> createState() => _SupportChatThreadState();
}

class _SupportChatThreadState extends ConsumerState<SupportChatThread> {
  ...
  Future<void> _sendText() async {
    ...
    await ref.read(supportActionsProvider).sendMessage(
  ...
  Future<void> _attach() async {
    ...
    try {
      final (url, type) = await ref.read(supportActionsProvider).uploadAttachment(file!.bytes!, file.name);
      await ref.read(supportActionsProvider).sendMessage(
            ticketId: widget.ticketId,
            senderRole: widget.myRole,
            body: '',
            ...
            children: [
              IconButton(
                ...
                icon: _isSending
                    ...
                    : const Icon(Icons.attach_file_rounded, color: VoltronColors.greyText),
              ),
              Expanded(
                child: TextField(
              ...
              IconButton(
                onPressed: _sendText,
                icon: const Icon(Icons.send_rounded, color: VoltronColors.electricYellow),
              ),
            ],
          ),
        ),
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isMine ? VoltronColors.electricYellow : VoltronColors.cardBlack,
            ...
            if (message.isImage && message.attachmentUrl != null)
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Icon(Icons.play_circle_fill_rounded, size: 40, color: VoltronColors.electricYellow),
                ),
              ),
